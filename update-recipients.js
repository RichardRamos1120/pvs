...
const backupPath = path.join(__dirname, 'src', 'components', 'SimpleGARAssessment.jsx.backup');
const content = fs.readFileSync(filePath, 'utf8');

// Create regex pattern to find the notifications section
const startPattern = '<div className="bg-gray-50 dark:bg-gray-750 rounded-md p-4 mb-6">';
const endPattern = '</div>'; // This is tricky because there are many closing divs

// Find where the notification section starts
const startIndex = content.indexOf(startPattern);
if (startIndex === -1) {
  console.error('Could not find notification section start pattern');
  process.exit(1);
}

// Find the notification section title to confirm we're in the right place
const titleSearch = content.substring(startIndex, startIndex + 200);
if (!titleSearch.includes('Notification Recipients')) {
  console.error('Found start pattern but it doesn\'t contain "Notification Recipients"');
  process.exit(1);
}

// Find the end of the section by matching nested divs
let openDivs = 1; // Start with 1 for the opening div we already found
let endIndex = startIndex;
let searchStartIndex = startIndex + startPattern.length;

// This will find the matching closing div by counting opening and closing divs
while (openDivs > 0 && endIndex < content.length) {
  // Find the next div open or close
  const nextOpenIndex = content.indexOf('<div', searchStartIndex);
  const nextCloseIndex = content.indexOf('</div>', searchStartIndex);
  ...
  if (nextOpenIndex !== -1 && (nextCloseIndex === -1 || nextOpenIndex < nextCloseIndex)) {
    // Found an opening div first
    openDivs++;
    searchStartIndex = nextOpenIndex + 4; // Move past the opening
  } else {
    // Found a closing div
    openDivs--;
    if (openDivs === 0) {
      // This is the closing div we want
      endIndex = nextCloseIndex + 6; // Include the closing </div>
    }
    searchStartIndex = nextCloseIndex + 6; // Move past the closing
  }
}

if (openDivs !== 0) {
  console.error('Failed to find matching closing div');
  process.exit(1);
}

// Extract the section we want to replace
const sectionToReplace = content.substring(startIndex, endIndex);
console.log(`Found section to replace (${sectionToReplace.length} chars)`);
