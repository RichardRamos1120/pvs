...
const GARAssessment = () => {
  ...
  // Notification recipient state - now displayed inline
  ...
  const fetchUsers = async () => {
    try {
      console.log("[DEBUG] Fetching real users from database");

      // Initialize users array
      let users = [];

      // Try to get the current user profile first
      if (auth.currentUser && firestoreOperations.getUserProfile) {
        try {
          const currentUser = await firestoreOperations.getUserProfile(auth.currentUser.uid);
          if (currentUser) {
            users.push({
              id: auth.currentUser.uid,
              name: auth.currentUser.displayName || currentUser.name || "Current User",
              email: auth.currentUser.email || "",
              station: currentUser.station || selectedStation,
              rank: currentUser.role || "Firefighter"
            });
            console.log("[DEBUG] Added current user to recipients");
          }
        } catch (err) {
          console.warn("[DEBUG] Could not get current user profile:", err);
        }
      }

      // Try to get a list of all assessments to extract users
      try {
        ...
        if (assessments && assessments.length > 0) {
          ...
          // Extract captain names from assessments
          const captains = assessments
            .filter(a => a.captain && typeof a.captain === 'string')
            .map(a => ({
              id: `captain-${a.id || Math.random().toString(36).substring(2, 9)}`,
              name: a.captain,
              email: `${a.captain.toLowerCase().replace(/\s+/g, '.')}@example.com`,
              station: a.station || selectedStation,
              rank: 'Captain'
            }));

          // Add only unique captains (by name)
          const captainNames = new Set();
          captains.forEach(captain => {
            if (!captainNames.has(captain.name)) {
              captainNames.add(captain.name);
              users.push(captain);
            }
          });

          console.log("[DEBUG] Added captains from assessments:", captainNames.size);
        }
      } catch (err) {
        console.warn("[DEBUG] Error getting assessments:", err);
      }

      // Try to get all users if the method exists
      let snapshot = null;
      try {
        if (firestoreOperations.getAllUsers) {
          snapshot = await firestoreOperations.getAllUsers();
          console.log("[DEBUG] Result from getAllUsers:", snapshot);
        }
      } catch (err) {
        console.warn("[DEBUG] Error calling getAllUsers:", err);
      }

      if (snapshot && Array.isArray(snapshot)) {
        // If getAllUsers returns an array of users directly
        users = snapshot;
        console.log("[DEBUG] Retrieved users from array:", users.length);
      } else if (snapshot && typeof snapshot === 'object' && snapshot.docs) {
        // If it returns a Firebase snapshot
        users = snapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        }));
        console.log("[DEBUG] Retrieved users from snapshot:", users.length);
      } else {
        console.warn("[DEBUG] No users found or invalid format returned");

        // Attempt to get users another way - via station assessments
        try {
          // Get assessments for this station to extract users
          const stationAssessments = await firestoreOperations.getAssessmentsByStation(selectedStation);
          if (stationAssessments && stationAssessments.length > 0) {
            console.log("[DEBUG] Found station assessments:", stationAssessments.length);

            // Extract unique captains from assessments
            const captainMap = new Map();
            stationAssessments.forEach(assessment => {
              if (assessment.captain) {
                const userId = `captain-${assessment.id}`;
                captainMap.set(assessment.captain, {
                  id: userId,
                  name: assessment.captain,
                  email: `${assessment.captain.toLowerCase().replace(/\s+/g, '.')}@example.com`,
            ...
            // Add unique captains to users array
            captainMap.forEach(captain => {
              users.push(captain);
            });

            console.log("[DEBUG] Added users from station assessments:", captainMap.size);
          }
        } catch (err) {
          console.error("[DEBUG] Failed to get users from station assessments:", err);
        }
      }

      // Don't create default users anymore to avoid confusion with dummy data
      if (!users || users.length === 0) {
        console.warn("[DEBUG] No users found in the database");
        
        // Return an empty array - no dummy data
        users = [];
      }
      // Format users for consistent data structure - but filter out example.com emails
      const formattedUsers = users
        .filter(user => !user.email || !user.email.includes('@example.com')) // Filter out dummy emails
        .map(user => ({
          id: user.id || user.uid || `user-${Math.random().toString(36).substring(2, 9)}`,
          name: user.displayName || user.name || `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'Unknown User',
          email: user.email || '', // Don't generate dummy emails
          station: user.station || 'Unknown Station',
          rank: user.role || user.rank || 'Firefighter' // Default to Firefighter if no role/rank
        }));

      // Make sure names are unique for UI identification
      const userMap = new Map();
      formattedUsers.forEach(user => {
        if (userMap.has(user.name)) {
          // Add a number if duplicate name exists
          let count = 1;
          let newName = `${user.name} (${count})`;
          while (userMap.has(newName)) {
            count++;
      ...
      console.log("[DEBUG] Set users for recipient selection:", formattedUsers.length);
    } catch (error) {
      console.error('[DEBUG] Error fetching users:', error);

      // Create fallback users in case of complete failure
      const fallbackUsers = stations.map((station, index) => ({
        id: `default-${index}`,
        name: `Default User ${index + 1}`,
        email: `user${index + 1}@firereporting.com`,
        station: station,
        rank: index % 3 === 0 ? 'Captain' : (index % 2 === 0 ? 'Lieutenant' : 'Firefighter')
      }));
  ...
  // Filter users based on search term and filters - simplified approach similar to TodayLog
  const filterUsers = () => {
    ...
    // Apply station filter first
    if (selectedStationFilter !== 'All') {
      filtered = filtered.filter(user =>
        user.station === selectedStationFilter
      );
    }
    ...
    if (selectedRankFilter !== 'All') {
      filtered = filtered.filter(user =>
        (user.rank || user.role || '').toLowerCase() === selectedRankFilter.toLowerCase()
      );
    }

    // Apply search term last - simplified to match TodayLog approach
    if (recipientSearchTerm && recipientSearchTerm.trim() !== '') {
      const search = recipientSearchTerm.trim().toLowerCase();
      filtered = filtered.filter(user =>
        (user.name || user.displayName || '').toLowerCase().includes(search) ||
        (user.email || '').toLowerCase().includes(search) ||
        (user.station || '').toLowerCase().includes(search)
      );
    }

    console.log(`[DEBUG] Filtered users: ${filtered.length} of ${allUsers.length} match criteria`);
    setFilteredUsers(filtered);
  };

  // Effect to filter users when search term or filters change
  useEffect(() => {
    filterUsers();
  }, [recipientSearchTerm, selectedStationFilter, selectedRankFilter, allUsers]);

  // Don't automatically fetch users when component mounts
  // Users will be loaded on-demand when the notification recipients section is expanded
  // or when the user clicks the "Refresh Users" button
  useEffect(() => {
    console.log("[DEBUG] Component mounted - users will be loaded on demand only");
    // No automatic loading to prevent unnecessary database calls
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Don't automatically fetch users - they'll be loaded on demand
        // This improves performance and prevents unnecessary database calls
  ...
  const closeAssessment = async () => {
    ...
    // Use direct browser navigation - this is what works
  ...
  const toggleRecipientGroup = (groupId) => {
    setAssessmentData(prev => {
      ...
      const updatedGroups = prev.notificationRecipients?.groups?.map(group => {
        if (group.id === groupId) {
  ...
  const toggleUserSelection = (userId) => {
    setAssessmentData(prev => {
      ...
      const userIndex = prev.notificationRecipients?.users?.findIndex(u => u.id === userId) || -1;
  ...
  const isUserSelected = (userId) => {
    return assessmentData?.notificationRecipients?.users?.some(u => u.id === userId) || false;
  };
  ...
  const publishAssessment = async () => {
    try {
      ...
      const sendNotifications = async () => {
        try {
          // Check if notification recipients exists and has the required properties
          if (!assessmentData.notificationRecipients || !assessmentData.notificationRecipients.groups) {
            console.error("[DEBUG] Invalid notification recipients structure:", assessmentData.notificationRecipients);

            // Initialize with defaults if missing
            if (!assessmentData.notificationRecipients) {
              setAssessmentData(prev => ({
                ...prev,
                notificationRecipients: {
                  groups: [],
            ...
            // Return true to prevent blocking publication
            return true;
          }

          // Get selected groups with safe handling
          const selectedGroups = assessmentData.notificationRecipients.groups
            .filter(group => group.selected)
            .map(group => group.name);

          // Get selected individual users with safe handling
          const selectedUsers = assessmentData.notificationRecipients.users || [];

          // In a real implementation, this would send emails using a server function
          console.log("[DEBUG] Sending notifications to groups:", selectedGroups);
          console.log("[DEBUG] Sending notifications to users:", selectedUsers);

          // SMTP configuration using the provided credentials
          const smtpConfig = {
            username: "MS_aZ8xjf@trial-xkjn41m2xv54z781.mlsender.net",
            password: "mssp.Ez4wEc1.o65qngkk9jdgwr12.bJebh3u",
          ...
          // Email Content
          const emailContent = {
            subject: `GAR Risk Assessment: ${riskLevel.level} - Score: ${totalScore}`,
            body: `
              <html>
              <body>
                <h2>Risk Assessment Notification</h2>
                <p><strong>Risk Level:</strong> ${riskLevel.level}</p>
                <p><strong>Score:</strong> ${totalScore}</p>
                <p><strong>Type:</strong> ${assessmentData.type || "N/A"}</p>
                <p><strong>Station:</strong> ${assessmentData.station || "N/A"}</p>
                <p><strong>Date:</strong> ${assessmentData.date || "N/A"}</p>
                <p><strong>Time:</strong> ${assessmentData.time || "N/A"}</p>
                ${assessmentData.weather?.alerts ? `<p><strong>Weather Alert:</strong> ${assessmentData.weather.alerts}</p>` : ''}
                <p>Please log in to view the full assessment.</p>
              </body>
              </html>
            `
          };

          // Log details - would be sent to server in real implementation
          console.log("[DEBUG] Using SMTP config:", smtpConfig);
          console.log("[DEBUG] Email content:", emailContent);

          // For now, we'll just log that notifications were sent
          console.log("[DEBUG] Notifications sent successfully via SMTP");
          ...
          // Still return true to prevent blocking publication
          return true;
        }
      };
      ...
      try {
        ...
        emailService.default.initEmailJS();
        console.log("[DEBUG] EmailJS initialized");

        // Get selected users with safe handling
        const selectedUsers = assessmentData.notificationRecipients?.users || [];
        console.log(`[DEBUG] Selected users for notification: ${selectedUsers.length}`);

        // Check if we have recipients
        if (selectedUsers.length === 0) {
          console.log("[DEBUG] No recipients selected, skipping notifications");
        } else {
          // Create a copy of the assessment data with ID and risk info for the emailService
          const assessmentWithId = {
            ...assessmentToPublish,
            id: id || 'new-assessment',
            totalScore: totalScore,
            riskLevel: riskLevel
          };

          // Send notifications using EmailJS
          console.log("[DEBUG] Sending notifications via EmailJS...");
          console.log("[DEBUG] Assessment data for email:", JSON.stringify(assessmentWithId).substring(0, 200) + "...");
          const result = await emailService.default.sendAssessmentNotifications(
            assessmentWithId,
            selectedUsers
          );

          if (result) {
            console.log("[DEBUG] Notifications sent successfully via EmailJS");
            alert("Assessment published and notifications sent successfully. Check recipient inbox (may be in spam folder).");
          } else {
            console.warn("[DEBUG] Failed to send notifications via EmailJS");
            alert("Assessment published but notification sending failed. Check console for details.");
          }
        }
      } catch (error) {
        console.error("[DEBUG] Error sending notifications:", error);
        alert("Assessment published but encountered an error sending notifications: " + error.message);
        // Continue with publishing even if notifications fail
      }
  ...
  const Step4 = () => {
    // Use useEffect to initialize notificationRecipients if needed
    // This prevents the "Cannot update a component while rendering" error
    useEffect(() => {
      if (!assessmentData.notificationRecipients) {
        console.log("[DEBUG] Initializing notification recipients in Step4");
        setAssessmentData(prev => ({
          ...prev,
          notificationRecipients: {
            groups: [],
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        {/* Use our new NotificationRecipientsSection component */}
