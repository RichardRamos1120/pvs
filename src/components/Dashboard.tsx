                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow">
                        ...
                        <div className="p-6">
                            ...
                            {(() => {
                                ...
                                const totalHours = Object.values(categories).reduce((sum: number, hours: number) => sum + hours, 0);

                                // Format categories for display
                                const formattedCategories = Object.entries(categories)
                                    .sort((a: any, b: any) => b[1] - a[1]) // Sort by hours (descending)
                                    .map(([category, hours]: [string, number]) => ({
                                        category,
                                        hours: hours.toFixed(1),
                                        percentage: (totalHours as number) > 0 ? ((hours / (totalHours as number)) * 100).toFixed(0) : 0
                                    }));
